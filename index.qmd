...
kable(head(season_information_data))
```
...
air_time_data <- 
whoverse %>% 
    group_by(era, air_time) %>% 
    summarise(
        episode_amount = length(episode)
    )
```
...
air_time_data %>% 
    ggplot(
        aes(
            x = air_time, y = episode_amount, fill = era
        )
    ) + 
    ...
    labs(
        x = "Time Episode Aired", 
        y = 'Amount of Episodes',
        title = 'Amount of Episodes Airing at Certain Times'
    ) 
```

The bar chart can show us that Classic Dr. Who seemed to air between 17:10 and 19:10, so we can conclude that the content seen in the Classic Era was suitable for minors. 

When looking at the Modern Dr. Who, episodes tend to are at different times, some air at 0:00, others at 8:00, and then there is one (or a small amount) of episodes that air at 20:00. According to the UK's broadcasting schedules, this show seems to be suitable for minors. 

The Revival Dr. Who episodes, as well as Sarah Jane Adventures, also fall outside of the UK's 'adult / non-minor suitable' broadcasting times. 

Things start to get different when looking at Torchwood: 

Torchwood seems to air at time laters than or equal to 21:00. We can conclude that this show's target audience may be young adults or older teens. 

Torchwood Web of Lies do not have an air date.

### How can you explain any shows in the Dr. Who universe which do not have airtimes provided?

Torchwood Web of Lies, unlike the other series, is animated. Not only that, but it is also not a TV show, it is instead a comic book that had "episodes" released weekly. In return, they do not have an air date on television provided. 
...
::: callout
I think I accidentally did this first with my `era_table` dataframe?
:::
...
I am just going to go with examining the frequency of words in the episode descriptions accross different series. I plan to do this by plotting the episodes by their air date and coloring/filling them by the era/spinoff/series they appeared in. I hope I can get a nice plot, better than the others, that fits all of the series into one plot nicely without sacrificing any information.
...
### Examining the Frequency of Words in the Episode Descriptions

```{r}
episode_word_count_data <- whoverse %>%
    select(era,air_date, summary) %>%
    mutate(
        summary = str_remove_all(summary, "<.*?>"),
    ) %>% 
    mutate(
        word_count = str_count(summary, "\\S+")
    )

kable(head(episode_word_count_data))
```

### Exploring it 

```{r}
summary(episode_word_count_data$word_count)
```
### Visual Plot

```{r, warning = F, message = F}
episode_word_count_data %>% 
    ggplot(aes(
        x = air_date, y = word_count, color = era, fill = era
    )) + 
    geom_point() + 
    labs(
        title = 'Amount of words in the descriptions of Dr. Who series/spinoffs over time',
        x = "Date when Episode Aired",
        y = "Word Count"
    )
```

The plot shows that as time went on and more and more Dr. Who series/spinoffs were made, the detail that was put into the episode description was less and less. 
